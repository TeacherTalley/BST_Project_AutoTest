...
def file_contains_file(file, searchfile, args=None):
    ...
    if args.verbose:
        print(f'Check file {file} contains file {searchfile}')
...
def file_contains_string(file, searchstring, args=None):
    ...
    if args.verbose:
        print(f'Check file {file} contains "{searchstring}"')
...
def file_contains_regex(file, searchstring, args=None):
    ...
    if args.verbose:
        print(f'Executing: Check file {file} contains regex "{searchstring}"')
    ...
    if re.search(searchstring, filedata):
        ...
        if args.verbose:
            print(f'ERROR: Regex "{searchstring}" not found in {file}')
            print(f'\nExpected:\nRegex {searchstring}')
            print(f'\nActual:\n{filedata}')
        return 1
...
def test_watch(args):
    ...
    if args.verbose:
        print(f'Checking watched movies {DEL_MOVIES} removed')
    ...
    if rc != 0:
        if args.verbose:
            print(f'ERROR: {DEL_MOVIES} not removed from {STUDENT_MOVIE_QUEUE_UPDATE_FILE}')
        return rc 
...
def test_delete(args):
    ...
    if args.verbose:
        print(f'Checking {DEL_MOVIES} deleted from updated queue')
...
def test_delete_not_found(args):
    ...
    # check that the updated movie queue file does not contain deleted movies
    if args.verbose:
        print(f'Checking movie list unchanged')
    ...
    if rc != 0:
        if args.verbose:
            print(f'ERROR: {STUDENT_MOVIE_QUEUE_UPDATE_FILE} is incorrect')
        return rc 
